...
titles with an ellipsis. If no Herdr window is attached, one is opened. A jump closes the panel:
the conversation has moved to the terminal, and Hermes allows one writer per
session.

It is offered only while the roster is read from **this machine**. A session id
from a remote instance names a session on that host, which no pane here can run -
so the action is not shown there rather than promising a jump it cannot make.
