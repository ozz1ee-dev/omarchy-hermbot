...
// silhouettes. That mapping and the "sun" case are the only new drawing code;
// the eye geometry, the pose table and the flourishes are Rakabot's unchanged.
//
// A bot's avatar: its own shape and colour, with eyes that sit on a sphere and
// a face that carries its state.
//
...
Item {
  ...
  // Hermes stores a free-form shape string in `ui_meta['hermes-bots']`: a bare
  // legacy shape, a blobatar string (`blobatar`, `blobatar:<seed>`,
  // `blobatar:<seed>:<kind>`, `blobatar::<kind>`) or `sigil-<n>`. Rakabot's
  // silhouettes are the drawing, so each name is mapped onto one of them; `sun`
  // is the one Rakabot never had and is drawn in path() below. A blobatar string
  // that pins no kind is the lumpy one, a blobatar seed changes nothing here
  // (Rakabot's shapes are not seeded), and a sigil keeps a plain round body.
  readonly property string silhouette: {
    var name = String(shape || "")
    if (name.indexOf("blobatar:") === 0) {
      var parts = name.split(":")
      name = parts.length >= 3 && parts[2] !== "" ? parts[2] : "organic"
    }
    if (name.indexOf("sigil-") === 0) return "circle"
    switch (name) {
    case "squircle": return "squircle"
    case "circle":
    case "round": return "blob"
    case "pill":
    ...
    // boxy is a rounded polygon and nub is a lobe on a round body; Rakabot's
    // nearest silhouettes stand in rather than inventing new geometry.
    case "boxy": return "tablet"
    case "organic": return "bean"
    case "nub": return "pebble"
